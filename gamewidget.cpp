...
void GameWidget::setupScene(int i){
    ...
    score=0;
    trans=0;
    QSound* wel=new QSound(":/music/Welcome.wav");
    wel->play();
    scoreTextLabel=new QLabel(this);
    scoreTextLabel->setGeometry(360,130,150,50);
    scoreTextLabel->setAlignment(Qt::AlignCenter);
    scoreTextLabel->setFont(QFont("Euclid",40,QFont::Bold));
    scoreTextLabel->setStyleSheet("QLabel{color:white;}");
    ...
    {
        QPalette pal1;
        pal1.setBrush(backgroundRole(),QBrush(QPixmap(":/image/background1.jpg").scaled(this->size(),Qt::IgnoreAspectRatio)));
        this->setPalette(pal1);
    }
    if(DIFFICULTY == 6 )
    {
    QPalette pal2;
    pal2.setBrush(backgroundRole(),QBrush(QPixmap(":/image/background5.jpg").scaled(this->size(),Qt::IgnoreAspectRatio)));
    this->setPalette(pal2);
    }
    if(DIFFICULTY == 7 )
    {
    QPalette pal3;
    pal3.setBrush(backgroundRole(),QBrush(QPixmap(":/image/background7.jpg").scaled(this->size(),Qt::IgnoreAspectRatio)));
    ...
    /*sound->setSource("src:/music/music-2.wav"));
    ...
    setWindowIcon(QIcon("://image/app_icon.ico"));
    ...
    //背景图片 棋盘框 分数框
    setAdaptedImage(":/image/backdrop00.jpg",ui->oriBkLbl);
    setAdaptedImage(":/image/setting/frame.png",ui->boardLbl);
    setAdaptedImage(":/image/setting/score.png",ui->scoreLbl);

    //设置变红的四周提示灯标签
    redLabel=new QLabel(this);
    redLabel->setGeometry(610, 2, 1055, 1073);
    setAdaptedImage(":/image/frame_red.png",redLabel);
    redLabel->setAttribute(Qt::WA_TransparentForMouseEvents);
    //辅助label，不用管
    ...
    menuButton = new Pbtn_Start(this);
    menuButton->setGeometry(ui->menuLbl->geometry());
    menuButton->setImage(":/image/setting/menu1.png",nullptr,ui->menuLbl->width(),ui->menuLbl->height(),ui->menuLbl);

    hintButton = new Pbtn_Start(this);
    hintButton->setGeometry(ui->hintLbl->geometry());
    hintButton->setImage(":/image/setting/hint.png",nullptr,ui->hintLbl->width(),ui->hintLbl->height(),ui->hintLbl);

    pauseButton = new Pbtn_Start(this);
    pauseButton->setGeometry(ui->pauseLbl->geometry());
    pauseButton->setImage(":/image/setting/pause.png",nullptr,ui->pauseLbl->width(),ui->pauseLbl->height(),ui->pauseLbl);

    resetButton = new Pbtn_Start(this);
    resetButton->setGeometry(ui->reSetBtn->geometry());
    resetButton->setImage(":/image/setting/reSetButton.png",nullptr,ui->reSetBtn->width(),ui->reSetBtn->height(),ui->reSetBtn);
    ...
    if(i==0){
        menuButton->showContent("",20);
        menuButton->show();
        hintButton->showContent("",30);
        hintButton->show();
        pauseButton->showContent("",20);
        pauseButton->show();
        resetButton->showContent("重置",20);
        resetButton->show();
    }
    //设置鼠标-普通
    ...
    //棋盘
    QPropertyAnimation* anim1 = new QPropertyAnimation(ui->boardLbl, "geometry");
    ...
    progressTimer->start();
    redBoardTimer=new QTimer(this);
    redBoardTimer->setInterval(500);
    timeoutTimer=new QTimer(this);
    timeoutTimer->setInterval(30);
    //timeoutTimer->start();
    connect(redBoardTimer, &QTimer::timeout, [=](){
        if(redBoardershow ==0){
            redLabel->show();
            redBoardershow=1;
        }else{
    ...
    connect(timeoutTimer, &QTimer::timeout, [=](){
        ...
        QPixmap input(":/image/setting/Game_over.png");
        ...
        if(!is_paused){
            if(progressBar->value() == 0){
                ...
                if(!timeoutTimer->isActive()){
                    ...
                    QMediaPlaylist *music_list1 = new QMediaPlaylist(this);
                    music_list1->addMedia(QUrl("qrc:/music/gameover.mp3"));
                    media->setPlaylist(music_list1);
                    music_list1->setPlaybackMode(QMediaPlaylist::CurrentItemOnce);
                    media->play();
                    /*
                    effect=new QSound(":/music/Time_Up.mp3");
                    effect->play();
                    */
                    //client->update(score);
                }
                if(redBoardTimer->isActive()){
                    redBoardTimer->stop();
                    redLabel->show();
                    redBoardershow=1;
                }
    ...
    connect(resetButton,&Pbtn_Start::clicked,[=]{
       if(is_acting||GameOver)
           return;
       reSetBoard();
    });

    connect(menuButton, &Pbtn_Start::clicked, [=](){
    ...
    connect(hintButton,&Pbtn_Start::clicked,[=](){
        if(GameOver)
            return;
        if(!is_acting&&hintArrowTimes>=6){
            ...
            hintLabel->show();
            setAdaptedImage(":/image/arrow.png",hintLabel);
    ...
    connect(pauseButton,&Pbtn_Start::clicked,[=]{
        if(GameOver)
            return;
        if(!is_acting){
            if(!is_paused)
            {
                ...
                forbidAll(true);
                pauseBKLabel = new QLabel(boardWidget);
                pauseBKLabel->setGeometry(0,0,952, 952);
                pauseBKgif = new QMovie("://image/starsBK.gif",QByteArray(),boardWidget);
                pauseBKgif->setScaledSize(QSize(952,952));
                pauseBKLabel->setMovie(pauseBKgif);
                pauseBKLabel->show();
                pauseBKgif->start();

                pauseTXLabel = new QLabel(boardWidget);
                pauseTXLabel->setGeometry(250,boardWidget->height()/2,440,30);
                pauseTXLabel->setText(tr("游戏暂停了，休息一下吧~"));
                pauseTXLabel->setAlignment(Qt::AlignCenter);
                pauseTXLabel->setFont(QFont("BoDoni MT",25, QFont::Normal));
                pauseTXLabel->setStyleSheet("QLabel{color:white;}");
                pauseTXLabel->setVisible(true);
                is_paused=true;
            }
            else if(is_paused){
                if(i==1){
...
void GameWidget::reSetBoard(){
    ...
    for(int j = 7; j >=0; --j){
        for(int i = 0; i <8 ; ++i){
            gemType[i][j] = gemType[i][j] % static_cast<unsigned int>(DIFFICULTY) + 1;
...
void GameWidget::setAdaptedImage(QString path,QLabel *label)
{
    QImage image(path);
...
int GameWidget::randomGem(){
    return QRandomGenerator::global()->bounded(1, DIFFICULTY+1);
}
...
void GameWidget::startGame(){
    ...
    connect(group, &QParallelAnimationGroup::finished, [=] {
        ...
        connect(this, &GameWidget::eliminateFinished, [=] {
            ...
            hit->play();
            scoreTextLabel->setText(QString::number(score));
            forbidAll(false);
            is_acting=false;
            int s = updateBombList();
            score+=s;
...
void GameWidget::act(Gem* gem){
    ...
    if(selectedX==-1 && selectedY==-1){
        ...
        selectedY = gemY;
        //加选框
        selectedLabel = new QLabel(boardWidget);
        QImage image("://image/selected.png");
        selectedLabel->setPixmap(QPixmap::fromImage(image));
        selectedLabel->setGeometry(len*selectedX, len*selectedY, len, len);
        selectedLabel->setAttribute(Qt::WA_TransparentForMouseEvents);
        selectedLabel->show();
        //让选中宝石旋转
    ...
    //如果有宝石选中，并点击了邻居宝石，则让宝石交换
        ...
        if(gems[SX][SY]->type==0){
            ...
            if(currentScore == 0) {
                if(effect)
                    delete effect;
                effect=new QSound(":/music/effect/bad.mp3");
                effect->play();
                std::swap(gemX,SX);
                std::swap(gemY,SY);
    ...
    if(type==0){
        ...
        connect(gather,&QParallelAnimationGroup::finished,[=]{
            ...
            if(tty%10==3){//魔方
                ...
                setAdaptedImage(gems[cX][cY]->path_stable[(tty-tty%10)/10],gems[cX][cY]->normalLabel);
...
void GameWidget::eliminateBoard(){
    ...
    for(unsigned int i=0;i<(bombList.size());i++){
        if(!bombList[i])
            bombList.erase(bombList.begin()+i);//删除元素后，后面元素自动往前移
    }
    QParallelAnimationGroup* biggerGroup = new QParallelAnimationGroup;
    for(unsigned int i=0;i<(bombList.size());i++){
